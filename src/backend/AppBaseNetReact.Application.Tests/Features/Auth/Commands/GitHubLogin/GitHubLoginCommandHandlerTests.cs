...
public class GitHubLoginCommandHandlerTests
{
    private readonly Mock<IGitHubAuthService> _githubAuth = new();
    private readonly Mock<IUnitOfWork> _uow = new();
    private readonly Mock<IJwtService> _jwt = new();
    private readonly Mock<IDateTimeProvider> _clock = new();
    private readonly Mock<ILogger<GitHubLoginCommandHandler>> _logger = new();
    private readonly Mock<IRoleRepository> _roleRepo = new();
    private readonly GitHubLoginCommandHandler _handler;

    private readonly GitHubUserInfo _userInfo = new("12345", "user@github.com", "John", "Doe");
    private const string Code = "auth-code";
    private const string State = "nonce";

    public GitHubLoginCommandHandlerTests()
    {
        ...
        _githubAuth.Setup(x => x.ExchangeCodeAsync(Code, State, It.IsAny<CancellationToken>()))
            .ReturnsAsync(_userInfo);

        _githubAuth.Setup(x => x.GetAuthorizationUrl(It.IsAny<string>())).Returns("https://github.com/login/oauth/authorize?client_id=xxx");

        _handler = new GitHubLoginCommandHandler(
            _githubAuth.Object, _uow.Object, _jwt.Object, _clock.Object, _logger.Object);
    }
    ...
    private static User CreateActiveUser(string email = "user@github.com")
    {
    ...
    [Fact]
    public async Task Handle_NewUserViaGitHub_CreatesUserAndExternalLogin()
    {
        _uow.Setup(x => x.ExternalLogins.GetByProviderAsync("github", _userInfo.ProviderId, It.IsAny<CancellationToken>()))
        ...
        var outcome = await _handler.Handle(
            new GitHubLoginCommand(Code, State, "127.0.0.1", "Mozilla/5.0", "http://localhost:5173"),
    ...
    {
        var existingUser = CreateActiveUser("user@github.com");
    ...
    {
        var user = CreateActiveUser("user@github.com");
        var externalLogin = ExternalLogin.Create(user.Id, "github", _userInfo.ProviderId, user.Email);
    ...
    public async Task Handle_InvalidState_ReturnsInvalidStateError()
    {
        ...
        outcome.Result.ErrorCode.Should().Be(GitHubLoginErrorCode.InvalidState);
    ...
    public async Task Handle_AuthFailed_ReturnsAuthFailedError()
    {
        ...
        outcome.Result.IsSuccess.Should().BeFalse();
        outcome.Result.ErrorCode.Should().Be(GitHubLoginErrorCode.AuthFailed);
        outcome.AccessToken.Should().BeNull();
    }
}
