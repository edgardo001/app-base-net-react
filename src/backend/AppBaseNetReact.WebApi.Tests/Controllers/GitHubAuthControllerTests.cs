...
public class GitHubAuthControllerTests
{
    ...
    private readonly Mock<ILogger<GitHubAuthController>> _logger = new();
    private readonly GitHubAuthController _controller;
    ...
    public GitHubAuthControllerTests()
    {
        ...
        _githubAuth.Setup(x => x.GetAuthorizationUrl(It.IsAny<string>()))
            .Returns("https://github.com/login/oauth/authorize?client_id=xxx");

        _controller = new GitHubAuthController(_githubAuth.Object, _mediator.Object, config, _logger.Object);
    ...
    public void Login_ReturnsRedirectToGitHub()
    {
        ...
        redirect.Url.Should().Be("https://github.com/login/oauth/authorize?client_id=xxx");
        redirect.Permanent.Should().BeFalse();
        _githubAuth.Verify(x => x.GetAuthorizationUrl(It.IsAny<string>()), Times.Once);
    }

    [Fact]
    public async Task Callback_WithValidCodeAndState_RedirectsToFrontendWithTokens()
    {
        var successOutcome = new GitHubLoginOutcome(
        ...
        _mediator.Setup(x => x.Send(
            It.Is<GitHubLoginCommand>(c => c.Code == "valid-code" && c.State == "valid-state"),
    ...
    public async Task Callback_WithMissingCode_RedirectsToLoginError()
    {
        ...
        var redirect = result.Should().BeOfType<RedirectResult>().Subject;
        redirect.Url.Should().Be("http://localhost:5173/login?error=github_auth_failed");
        _mediator.Verify(x => x.Send(It.IsAny<GitHubLoginCommand>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    {
        var failOutcome = new GitHubLoginOutcome(
            GitHubLoginResult.Fail(GitHubLoginErrorCode.AuthFailed, "GitHub authentication failed"));

        _mediator.Setup(x => x.Send(It.IsAny<GitHubLoginCommand>(), It.IsAny<CancellationToken>()))
