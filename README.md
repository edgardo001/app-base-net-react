...
graph TD
    ...
    subgraph Application["Application (Casos de Uso)"]
        ...
        CQRS["📁 CQRS<br/>Commands/ Queries/<br/>Handlers (Auth, Users, Roles ✅<br/>Permissions 🏗️)"]
    end
...
Los módulos **Auth**, **Users** y **Roles** están migrados a CQRS con handlers MediatR (`Application/Features/`). Los controllers inyectan solo `IMediator` y delegan la lógica de negocio a los handlers. Los módulos restantes (Permissions, Profile, Dashboard, Audit) aún usan `IUnitOfWork` directamente en los controllers.
