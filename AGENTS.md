...
| **Orquestación de negocio (CreateUser — token + email confirmation)** | ⚡ `UsersController.CreateUser` (controller orquesta, genera password temporal vía `IRandomPasswordGenerator`, genera token + envía `EmailConfirmation` con contraseña + `ForcePasswordChange`) — fixed en `openspec/changes/2026-06-06-user-creation-email-confirmation/` y extendido en `openspec/changes/2026-06-06-secure-user-onboarding/`. Spec de capacidad: `openspec/specs/user-creation/spec.md` (10 requirements: 5 de email-confirmation, 2 de frontend-link, 3 de secure-onboarding — no admin password, temp password in email, resend refuses confirmed). | 🎯 Migrar a `Application/Features/Users/Commands/CreateUser/CreateUserCommandHandler.cs` con `CreateUserCommand`, `CreateUserCommandValidator`, `CreateUserOutcome` — separado de `auth-confirm-email` (que ya provee el lado de confirmación). |
