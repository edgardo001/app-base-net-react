...
| **Orquestación de negocio (CreateUser — token + email confirmation)** | ⚡ `UsersController.CreateUser` (controller orquesta, genera password temporal vía `IRandomPasswordGenerator`, genera token + envía `EmailConfirmation` con contraseña + `ForcePasswordChange`) — fixed en `openspec/changes/2026-06-06-user-creation-email-confirmation/`, extendido en `openspec/changes/2026-06-06-secure-user-onboarding/`, y protegido con partial unique indexes en `openspec/changes/2026-06-06-no-user-delete-partial-unique-email/` (no hay ruta de borrado, `IX_Users_Email` y `IX_Users_NormalizedEmail` son `WHERE "DeletedAt" IS NULL`). Spec de capacidad: `openspec/specs/user-creation/spec.md` (12 requirements: 5 de email-confirmation, 2 de frontend-link, 3 de secure-onboarding, 2 de no-delete + partial-unique). | 🎯 Migrar a `Application/Features/Users/Commands/CreateUser/CreateUserCommandHandler.cs` con `CreateUserCommand`, `CreateUserCommandValidator`, `CreateUserOutcome` — separado de `auth-confirm-email` (que ya provee el lado de confirmación). |
