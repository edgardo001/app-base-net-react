...
| **Orquestación de negocio (CreateUser — token + email confirmation)** | ⚡ `UsersController.CreateUser` (controller orquesta, genera token + envía `EmailConfirmation`) — fixed en `openspec/changes/2026-06-06-user-creation-email-confirmation/` (cambio solo de comportamiento, no CQRS; el endpoint sigue en el controller). Spec de capacidad: `openspec/specs/user-creation/spec.md` (5 requirements). | 🎯 Migrar a `Application/Features/Users/Commands/CreateUser/CreateUserCommandHandler.cs` con `CreateUserCommand`, `CreateUserCommandValidator`, `CreateUserOutcome` — separado de `auth-confirm-email` (que ya provee el lado de confirmación). |
